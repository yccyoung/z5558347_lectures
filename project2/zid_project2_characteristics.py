...
from project2 import util as util
...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    df_day_return = ret[ret_freq_use[0]] #在这里使用？
    df_day_return.index = pd.to_datetime(df_day_return.index)

    df_month_count = df_day_return.resample('ME').count()
    df_month_count_None = df_month_count.astype(object).where(df_month_count >= 18, None)

    monthly_std = df_day_return.resample('ME').std(ddof=0)
    # monthly_std = monthly_std.astype(object) # ⭐️ 关键一步：转换为 object 类型，允许 None 出现
    for col, series in df_month_count_None.items():
        for idx, val in series.items():
            if val is None:
                monthly_std.loc[idx, col] = np.nan #问是否是None?
    monthly_std.columns = [col + f'_{cha_name}' for col in monthly_std.columns]#cha_name在这里使用？
    monthly_std.index.name = 'Year_Month'
    monthly_std.dropna(how='all',inplace=True)
    monthly_std.index = monthly_std.index.to_period('M')
...
def merge_tables(ret, df_cha, cha_name):
    ...
    # <COMPLETE THIS PART>
    # 假设cha_name="vol"
    df_monthly_return = ret["Monthly"]

    df_cha_shifted = df_cha.shift(1)

    df_merged = pd.concat([df_monthly_return, df_cha_shifted], axis=1)
...
def cha_main(ret, cha_name, ret_freq_use: list):
    ...
    return merge_tables(ret, vol_cal(ret, cha_name, ret_freq_use),cha_name)
